...
if [ -z "$PS1" ]; then
  echo "ERROR: must call this script using \"source ~/bin/conda_install_openvisus.sh [version]\")"
  exit 1
fi
...
echo "activating conda env ${package}${env_version_str}..."
. ~/bin/conda_activate.sh ${package}${env_version_str}
